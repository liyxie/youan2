<template>
  ...
  <div>
    <el-form inline class="searchForm">
      <el-form-item>
        <el-input v-model="params.customerName" placeholder="供应商名称" style="width: 120px;"  clearable></el-input>
      </el-form-item>
      <el-form-item>
      ...
      <el-form-item>
        <el-button type="primary" @click="getCustomerPageList">
          <el-icon>
  ...
  <!-- 表格 -->
  <el-table :data="customerPageList" style="width: 100%; margin-top: 10px;" table-layout="auto" size="large" border stripe id="elTable">
    <el-table-column type="index" width="50" />
    <el-table-column prop="customerName" label="客户名称" sortable />
    <el-table-column prop="concat" label="客户联系人" sortable />
    <el-table-column prop="phone" label="客户联系电话" sortable />
    <el-table-column prop="address" label="客户地址" sortable />
    <el-table-column prop="createTime" label="创建时间" sortable />
    <el-table-column label="操作">
      <template #default="props">
        <el-button type="primary" title="修改" :icon="Edit" circle @click="openCustomerUpdate(props.row)" />
        <el-button type="danger" title="删除" :icon="Delete" circle @click="deleteCustomer(props.row.supplyId)" />
      </template>
    </el-table-column>
  </el-table>
  <!-- 分页 -->
  ...
  <!-- 添加供应商对话框 -->
  <customer-add ref="customerAddRef" @ok="getCustomerPageList"></customer-add>

  <!-- 修改供应商对话框 -->
  <customer-update ref="customerUpdateRef" @ok="getCustomerPageList"></customer-update>
</template>
...
const getCustomerPageList = () => {
  console.log(params)
  get("/customer/customer-page-list", params).then(result => {
    customerPageList.value = result.data.resultList;
...
const openCustomerAdd = () => {
  customerAddRef.value.open();
};


// 跳向修改供应商
import customerUpdate from "./customer-update.vue";
const customerUpdateRef = ref();
const openCustomerUpdate = (customer) => {
  customerUpdateRef.value.open(customer);
};


// 删除供应商
const deleteCustomer = (customerId) => {
  del(`/customer/customer-delete/${customerId}`, null, { title: "提示", message: "您确定删除该客户吗？" }).then(result => {
